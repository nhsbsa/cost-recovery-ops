...
router.post(/which-entitlement/, function(req, res){
  res.redirect('/version-45/s1/s072-registration/which-s1-entitlement');
})
...
router.post([/which-s1-entitlement/], function(req, res) {
  ...
  if (s1EntitlementType === 'E109') {
    res.redirect('/version-45/s1/s072-registration/entitlement-details');
  } else {
    res.redirect('/version-45/s1/s072-registration/entitlement-for');
  }
})
...
router.post([/entitlement-details/], function(req, res){
  res.redirect('/version-45/s1/s072-registration/search-institution-ID');
})

// Search by institution ID (to show functionality)
router.post([/search-institution-ID/], function(req, res){
  res.redirect('/version-45/s1/s072-registration/institution-ID-search-results');
})

// Search by institution name (to show functionality)
router.post([/search-institution-name/], function(req, res){
  res.redirect('/version-45/s1/s072-registration/institution-name-search-results');
})

// Institution ID search results
router.post([/institution-ID-search-results/], function(req, res){
  res.redirect('/version-45/s1/s072-registration/new-s1-s073-entitlement-cya');
})
...
router.post([/institution-ID-search-results-none/], function(req, res){
  res.redirect('/version-45/s1/s072-registration/add-new-institution-details');
})
...
router.post([/add-new-institution-details/], function(req, res){
  res.redirect('/version-45/s1/s072-registration/new-s1-s072-entitlement-cya');
})

// Check your answers
router.post([/new-s1-s072-entitlement-cya/], function(req, res){
  req.session.data['dr-add-s1-entitlement'] = 'yes'
  res.redirect('/version-45/s1/s072-registration/confirmation-s1-s072-added');
})
