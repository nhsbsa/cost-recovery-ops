...
router.post([/uk-claims-search-actual-cost/], function(req, res) {
  ...
  res.redirect('/version-38/uk-claims/uk-claims-search-results');
})
...
router.post(['/uk-claims-search-average-cost/'], function(req, res) {
  ...
  res.redirect('/version-38/uk-claims/uk-claims-search-results-found');
})
...
router.post([/uk-claims-search-by-id/], function(req, res) {
  ...
  res.redirect('/version-38/uk-claims/uk-claims-search-by-id-results');
})
...
router.get([/create-new-uk-claim/], function(req, res) {
  ...
  res.render('version-38/uk-claims/create-new-uk-claim', {
...
router.post([/create-new-uk-claim/], function(req, res) {
  ...
  if (isDuplicateClaim) {
    ...
    return res.redirect('/version-38/uk-claims/create-new-uk-claim');
  } else {
    // Clear errors
    req.session.data['duplicateClaimError'] = null;
    req.session.data['supplementalClaimError'] = null;
    return res.redirect('/version-38/uk-claims/check-claim-details');
  }
});

// Check claim details
router.get([/check-claim-details/], function(req, res) {
  res.render('version-38/uk-claims/check-claim-details', {
...
router.post([/check-claim-details/], function(req, res) {
  ...
  res.redirect('/version-38/uk-claims/uk-claims-loading-new-claim');
})
...
router.post([/uk-claims-loading-new-claim/], function(req, res) {
  ...
  if (entitlementArticleType === 'EHIC and PRC - Article 62' || entitlementArticleType === "S2/E112 - Article 20.2") {
    res.redirect('/version-38/uk-claims/s1-claim-forms');
  } else {
   res.redirect('/version-38/uk-claims/s1-claim-summary');
  }
});
