...
router.post('/continue-generating-s071', function(req, res) {
  ...
  if (continueToGenerateS071 === 'Yes') {
    ...
    res.redirect('/version-46/s1/account/s1-requests')
  }
})
...
router.post('/enter-date-of-uk-residency', function(req, res) {
  ...
  res.redirect('/version-46/s1/account/generate-s071/select-citizen-status');
})
...
router.post('/select-citizen-status', function(req, res) {
  ...
  res.redirect('/version-46/s1/account/generate-s071/enter-member-state-details');
})
...
router.post('/enter-member-state-details', function(req, res) {
  ...
  res.redirect('/version-46/s1/account/generate-s071/search-for-institution-by-id');
})

// Search for the institution by ID
router.post('/search-for-institution-by-id', function(req, res) {

  res.redirect('/version-46/s1/account/generate-s071/search-for-institution-by-id-results');
})

// Search for the institution by name
router.post('/search-for-institution-by-name', function(req, res) {

  res.redirect('/version-46/s1/account/generate-s071/search-for-institution-by-name-results');
})
...
router.post('/enter-entitlement-period', function(req, res) {
  ...
  res.redirect('/version-46/s1/account/generate-s071/cya-generate-s071');
})
...
router.post('/cya-generate-s071', function(req, res) {
  ...
  res.redirect('/version-46/s1/account/generate-s071/confirmation-s071-generated');
})

// View new S071 in the 'Documents' section
router.post('/confirmation-s071-generated', function(req, res) {

  res.redirect('/version-46/s1/account/generate-s071/review-institution-details');
})

// Check your answers before generating the S071
router.post('/review-institution-details', function(req, res) {
  const sendS071ToMemberState = req.session.data['send-s071-to-member-state']

  if (sendS071ToMemberState === 'Yes') {
    // Mark S071 as sent to MS
    req.session.data['s071-sent-to-ms'] = 'Yes'
    res.redirect('/version-46/s1/account/generate-s071/confirmation-s071-sent-to-ms')
  } else {
    res.redirect('/version-46/s1/account/system-generated-documents')
  }
})

// Confirmation S071 was sent to the member state
router.post('/confirmation-s071-sent-to-ms', function(req, res) {
...
router.get('/cancel-generate-s071', function (req, res) {
  ...
  res.render('version-46/s1/account/generate-s071/cancel-generate-s071')
})
